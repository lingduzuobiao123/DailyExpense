package com.dong.expense.ui.view;
...
import com.dong.expense.infos.GesturePoint;
...
public class GestureDrawline extends View {
	...
	public GestureDrawline(Context context, List<GesturePoint> list,
		...
		super(context);
		screenDispaly = ExpenseUtil.getScreenDispaly(context);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (currentPoint != null) {
				currentPoint.setPointState(ConstantPool.POINT_STATE_SELECTED);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (currentPoint == null && pointAt == null) {
				...
				if (currentPoint == null) {// 先判断当前的point是不是为null
					...
					currentPoint
							.setPointState(ConstantPool.POINT_STATE_SELECTED);
					passWordSb.append(currentPoint.getNum());
				}
			}
			if (pointAt == null
					|| currentPoint.equals(pointAt)
					|| ConstantPool.POINT_STATE_SELECTED == pointAt
							.getPointState()) {
				...
				pointAt.setPointState(ConstantPool.POINT_STATE_SELECTED);

				// 判断是否中间点需要选中
				GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint,
						pointAt);
				if (betweenPoint != null
						&& ConstantPool.POINT_STATE_SELECTED != betweenPoint
								.getPointState()) {
					// 存在中间点并且没有被选中
		...
		case MotionEvent.ACTION_UP:// 当手指抬起的时候
			if (isVerify) {
				...
				if (MD5.validatePassword(passWord, passWordSb.toString())) {
					// 代表用户绘制的密码手势与传入的密码相同
	...
	final class clearStateRunnable implements Runnable {
		public void run() {
			...
			for (GesturePoint p : list) {
				p.setPointState(ConstantPool.POINT_STATE_NORMAL);
			}
			invalidate();
			isDrawEnable = true;
		}
	...
	private void drawErrorPathTip() {
		...
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
			pair.first.setPointState(ConstantPool.POINT_STATE_WRONG);
			pair.second.setPointState(ConstantPool.POINT_STATE_WRONG);
			canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
					pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
		}
		invalidate();
